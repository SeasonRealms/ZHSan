    ...
    public class ButtonTexture : IFrameContent
    {
        ...
        public void DrawToCanvas(SpriteBatch batch)
        {
            if (Visible)
            {
                ...
                Texture2D tex = CacheManager.LoadTexture(Text);
                if (tex != null && !tex.IsDisposed)
                    batch.Draw(tex, drawPos * DrawScale, Rectangle, color * Alpha, 0f, Vector2.Zero, Scale, SpriteEffects.None, Depth);
                ...
                if (!String.IsNullOrEmpty(ViewText))
                {
                    ...
                    List<Bounds> textBounds = CacheManager.DrawStringReturnBounds(batch, Session.Current.Font, Text, drawPos * DrawScale, (MouseOver || Selected) ? ViewTextColor2 * Alpha : ViewTextColor1 * Alpha, 0f, Vector2.Zero, Scale * ViewTextScale, SpriteEffects.None, Depth);
                    //处理文字边界范围
