    ...
    public class CheckBox : IFrameContent
    {
        ...
        public void DrawToCanvas(SpriteBatch batch)
        {
            if (Visible)
            {
                ...
                Vector2 drawPos = baseFrame == null ? Position : baseFrame.CanvasToScreen(Position);

                // SeasonXNA：繪製期未命中的紋理加載已延遲到下一幀（CacheManager.DrainPendingTextures），本輪暫不繪製。
                Texture2D tex = CacheManager.LoadTexture(Path);
                if (tex != null && !tex.IsDisposed)
                    batch.Draw(tex, drawPos, cbRectangle, color * Alpha, 0f, Vector2.Zero, Scale, SpriteEffects.None, Depth);
                ...
                bounds = CacheManager.DrawStringReturnBounds(batch, ViewFont, Text, (Vector2)(drawPos + _offsetText) * DrawScale, (MouseOver || Selected) ? ViewTextColorMouseOver * Alpha : ViewTextColor * Alpha, 0f, Vector2.Zero, Scale * ViewTextScale, SpriteEffects.None, Depth);
                ...
                if (AlignTexts.Count > 0)
                {
                    ...
                    AlignTexts.ForEach(at =>
                    {
                        _b = CacheManager.DrawStringReturnBounds(batch, ViewFont, at.Text, (Vector2)(drawPos + _offsetText + at.Offset) * DrawScale, (MouseOver || Selected) ? ViewTextColorMouseOver * Alpha : ViewTextColor * Alpha, 0f, Vector2.Zero, Scale * ViewTextScale, SpriteEffects.None, Depth);
                        if (_b.Count > 0)//处理Text为空的情况
                            b.Add(_b[0]);
                    });
                    //添加范围
