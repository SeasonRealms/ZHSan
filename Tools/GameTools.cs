...
// B0：原 SharpCompress（ZIP 讀寫）改用內置 System.IO.Compression，詳見 ZipFile 註釋。
...
namespace Tools
{
    ...
    public static class GameTools
    {
        ...
        {
            // B0：原 SharpCompress（BZip2 ZIP 寫入）未移植依賴，改用內置 System.IO.Compression.ZipArchive（Deflate）。
            // 語義等價：entry 名沿用 filePath、返回 zip 流長度；均為標準 zip，讀取方無感。
                ...
                using (var archive = new ZipArchive(zip, ZipArchiveMode.Create, leaveOpen: true))
                {
                    foreach (var filePath in files)
                    {
                        ...
                        using (Stream stream = Platform.Current.LoadUserFileStream(fileName, false))
                        {
                            if (stream != null)
                            {
                                // B0：改用無參重載（BCL 默認即 Optimal；本地 zlib 移植在同名命名空間定義了 CompressionLevel，會遮蔽 BCL 枚舉）。
        ...
        */
        ...
        {
            // B0：程序化圓形紋理改由平台 NativeImages 以 CPU 像素生成（PNG 編碼 → 引擎紋理）。
            if (shape == TextureShape.Circle)
            {
                return Zhsan.GameManager.NativeImages.Circle(Convert.ToInt32(shapeParms[0]), color);
            }
            return null;
        }
        ...
        {
            // B0：SeasonXNA 紋理是只讀句柄，不能就地改寫像素；Circle 遮罩由平台
            // NativeImages.LoadFile 在 CPU 解碼階段完成（當前項目無 Circle 調用點）。
#if false
            if (shape == TextureShape.Circle)
            {
