        ...
        public void LoadTextures()
        {
            ...
            try
            {
                foreach (TerrainDetail detail in Session.Current.Scenario.GameCommonData.AllTerrainDetails.TerrainDetails.Values)
                {
                    ...
                    // 目錄不存在時 GetFiles 返回 null（桌面文件系統與 Android 資產均如此）：
                    // 按空列表繼續，該地形無 Basic 紋理，但不中斷整體加載。
                    foreach (string str2 in Platform.Current.GetFiles(str).NullToEmptyArray())   // Directory.GetFiles(str))
                    {
