    ...
    public class MainMapLayer
    {
        ...
        private void CheckMapTileTexture(MapTile maptile)
        {
            if (maptile.TileTexture == null)
            {
                ...
                {
                    // B0：SeasonXNA Texture2D 無公共構造器，改用 NativeImages 生成 2x2 白色占位紋理。
                    maptile.TileTexture = Zhsan.GameManager.NativeImages.Circle(1, Microsoft.Xna.Framework.Color.White);
                    //try
                    //{
                ...
                MapThread1 = new PlatformTask(() =>
                {
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            if (mapTile != null)
                            {
                                // SeasonXNA：紋理創建須在幀線程且非繪製期（Game1.AssertResourceThread）；
                                // 後台線程改為把加載排到幀線程 Update 階段（原直調會被斷言吞掉、地圖塊永久缺失）。
                                var target = mapTile;
                                PlatformBase.Game1?.RunOnFrameThread(() =>
                                {
                                    if (target.TileTexture == null) CheckMapTileTexture(target);
                                });
                            }
                ...
                MapThread2 = new PlatformTask(() =>
                {
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                mapTile = maps.FirstOrDefault();
                                // SeasonXNA：同上，後台線程改走幀線程隊列加載。
                                var target = mapTile;
                                PlatformBase.Game1?.RunOnFrameThread(() =>
                                {
                                    if (target != null && target.TileTexture == null) CheckMapTileTexture(target);
                                });
                            }
