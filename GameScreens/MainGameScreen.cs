...
namespace WorldOfTheThreeKingdoms.GameScreens
{
    public partial class MainGameScreen : Screen
    {
            ...
            : base()
        {
            ...
            // B0：MonoGame Game.Activated/Deactivated 事件已不存在；激活/去激活改由 Update 輪詢 Platform.IsActive（見 Update 開頭）。
        }        
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public void DrawFrameRate()
        {
            ...
            Platform.Game1.Title = str;  // B0：Window.Title 不存在，改寫宿主 Title（引擎當前僅在啟動時應用一次）。
        }
        ...
        {
            // 分辨率由宿主維護（Game1.DesignResolution），此處不再讀取 MonoGame Window 客戶區。
            var host = Platform.Game1;
            Platform.Current.SetGraphicsWidthHeight(
                host != null ? (int)host.DesignResolution.X : Session.ResolutionX,
                host != null ? (int)host.DesignResolution.Y : Session.ResolutionY);
        ...
        // B0：輪詢宿主激活狀態的邊沿檢測（替代 MonoGame Game.Activated/Deactivated）。
        private bool lastWindowActive = true;

        public override void Update(GameTime gameTime)   //视野内容更新
        {
            bool windowActive = Platform.IsActive;
            ...
            if (toggleScreen)
            {
                ...
                if (toggleScreenTime >= 1f)
                {
                    ...
                    // 分辨率由宿主維護（Game1.DesignResolution），此處不再讀取 MonoGame Window 客戶區。
                    var host = Platform.Game1;
                    int width = host != null ? (int)host.DesignResolution.X : Session.ResolutionX;
                    int height = host != null ? (int)host.DesignResolution.Y : Session.ResolutionY;
                    Session.RealResolution = Session.Resolution = width + "*" + height;
                    Platform.Current.SetGraphicsWidthHeight(width, height);
                    //Session.MainGame.ToggleFullScreen();
                    Platform.Current.GraphicsApplyChanges();
                    this.UpdateViewport();
                    this.ResetTiles();
                    this.RefreshDisableRects();
                    JumpToFaction();
        ...
        private void UpdateViewMove()          //更新视野移动方向
        {
            ...
            if (Platform.PlatFormType == PlatFormType.Win || Platform.PlatFormType == PlatFormType.Desktop)
            {
                ...
                if (((Platform.IsActive && base.EnableScroll) && (!this.DrawingSelector && (base.viewportSize != Point.Zero))) && ((((InputManager.PoX >= 0) && (InputManager.PoY >= 0)) && (InputManager.PoX <= this.viewportSize.X)) && (InputManager.PoY <= this.viewportSize.Y)))
                {
        ...
        {
            // B0：SeasonXNA 不暴露 GraphicsDevice；視口尺寸取宿主設計分辨率（與 Update 中同模式）。
            var host = Platform.Game1;
            int viewWidth = host != null ? (int)host.DesignResolution.X : Session.ResolutionX;
            int viewHeight = host != null ? (int)host.DesignResolution.Y : Session.ResolutionY;

            if (viewWidth > 0 && viewHeight > 0)
            {
                if (Platform.PlatFormType == PlatFormType.Win || Platform.PlatFormType == PlatFormType.Desktop)
                {
                    this.viewportSize.X = viewWidth;
                    this.viewportSize.Y = viewHeight - this.Plugins.ToolBarPlugin.Height;
                    this.viewportSizeFull.X = viewWidth;
                    this.viewportSizeFull.Y = viewHeight;
                }
                else
                {
                    this.viewportSize.X = Session.ResolutionX - 20;
                    this.viewportSize.Y = Convert.ToInt32(Session.ResolutionY - this.Plugins.ToolBarPlugin.Height - 10);
                    // B0：移動端 DesignResolution 為物理像素，而界面（含 GameFrame 內容流）一律按邏輯坐標繪製與命中：
                    // viewportSizeFull 若沿用物理值，SetFrameContent 的居中定位會被 Scale2 再放大，令幀體偏向右下乃至出屏。
                ...
                Session.ChangeStartDisplay(viewWidth, viewHeight);
            }
        }
        ...
        public bool IsFullScreen
        {
            get
            {
                // B0：SeasonXNA 不暴露 GraphicsDevice；全屏狀態以設置中的 FullScreen 標誌為準。
                return Session.GlobalVariables.FullScreen;
            }
        }
