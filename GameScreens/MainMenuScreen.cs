...
namespace WorldOfTheThreeKingdoms.GameScreens
{
    ...
    public class MainMenuScreen
    {
        ...
        // 关于页文字内容是否已挂载（延迟到首次打开 About 时，见 EnsureAboutContent）。
        bool aboutContentReady = false;
        ...
        public MainMenuScreen()
        {
            ...
            // 关于页文字内容延迟到首次打开 About 时挂载（见 EnsureAboutContent）：
            // TextContent 构造与 AddContentContorl（内部计算控件尺寸）都会触发字体加载，
            // 放在构造期会让首帧同步读取字体文件（启动黑屏根因之一）。
        ...
        /// <summary>
        /// 关于页文字内容延迟挂载：TextContent 构造与 AddContentContorl（内部计算控件尺寸）
        /// 都会触发字体加载，放在构造期会让首帧同步读取字体文件（启动黑屏根因之一），
        /// 改为首次打开 About 时一次性挂载。
        /// </summary>
        void EnsureAboutContent()
        {
            if (aboutContentReady) return;
    ...
    nstFollowAttachPlus, nstFollowDefencePlus, nstSearchTime, nstGeneralChildsMax, nstGeneralChildsAppear, nstGeneralChildsSkill, nstChildsSkill, nstForbiddenDays };
        ...
        public void Draw(GameTime gameTime)
        {
            ...
            if (MenuType == MenuType.None)
            {
                // 字体后台预热未就绪时先只显示背景图，避免首次绘制触发同步字体加载。
                if (Zhsan.GameManager.MenuFontWarmup.Ready)
                {
    ...
    nstFollowAttachPlus, nstFollowDefencePlus, nstSearchTime, nstGeneralChildsMax, nstGeneralChildsAppear, nstGeneralChildsSkill, nstChildsSkill, nstForbiddenDays };
            ...
            else if (MenuType == MenuType.Start)
            {
                ...
                // 字体后台预热未就绪时只显示背景图，避免首跑开场白触发同步字体加载。
                if (Zhsan.GameManager.MenuFontWarmup.Ready)
                {
