    ...
    public static class InputManager
    {
        ...
        public static void Update(float gameTime)
        {
			if (Platform.IsActive)
            {
                ...
                // Pointer and keyboard state come from the engine pump (TouchService and the
                // SeasonXNA Mouse/Keyboard facades, refreshed once per frame by BaseApp.Update);
                // this single path now runs on every platform (mobile gestures arrive later).
                {
                    ...
                    if (Scale != ScaleOne)
                    {
                        PoX = Convert.ToInt32(OriginPoX / Scale.X);
                        PoY = Convert.ToInt32(OriginPoY / Scale.Y);
                    }
                    else
                    {
                ...
                // Touch-panel gesture path (Android/iOS/UWP). Superseded by the unified engine
                // pump above; re-enabled once multi-touch gestures are ported onto TouchService.
#if false
