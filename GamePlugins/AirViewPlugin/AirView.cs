        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (this.IsMapShowing)
            {
                ...
                if (this.MapTexture != null)
                {
                    ...
                    // SeasonXNA：D01-A 繪製契約要求 premultiplied tint（RGB ≤ Alpha），原版 straight(1,1,1,t)
                    // 在 t<1 時非法；Color.White * t 即其預乘表示（插件配置 Transparent=0.9 時不再崩）。
                    CacheManager.Draw(this.MapTexture, this.MapPosition, sourceRectangle, Color.White * this.Transparent, 0f, Vector2.Zero, SpriteEffects.None, 0.1f);
                }
                /*
